...
public class MachineMapper implements Mapper {
    private volatile Map<MachineMapperKey, Map<Class<? extends EventListener>, Machine>> machineMap = new ConcurrentHashMap<>();

    @Override
    public Map<Class<? extends EventListener>, Machine> getMachine(MachineMapperKey mapperKey) {
        return isExists(mapperKey) ? new HashMap<>(machineMap.get(mapperKey)) : Collections.EMPTY_MAP;
    }
